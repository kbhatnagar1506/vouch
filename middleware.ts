...
const PROTECTED_PAGE_PREFIXES = ["/voice"];
const PROTECTED_API_PREFIXES = ["/api/voice"];
...
export async function middleware(request: NextRequest) {
  ...
  if (!session) {
    ...
    // see docs/VOICE.md "Multi-tenancy" for the shared-session contract.
...
export const config = {
  matcher: ["/voice/:path*", "/api/voice/:path*"],
};
